...
const busyEventUIDs = new Set(); // events here ALWAYS block (unless whitelisted)
const calendarColors = {}; // calendar url -> color hex
...
async function loadPersistedConfigIfNeeded() {
    ...
    try {
        ...
        if (doc) {
            ...
            doc.busyCalendars.forEach((u) => busyCalendarUrls.add(u));
            whitelistUIDs.clear();
            doc.whitelistUIDs.forEach((u) => whitelistUIDs.add(u));
            busyEventUIDs.clear();
            doc.busyEventUIDs.forEach((u) => busyEventUIDs.add(u));
            Object.assign(calendarColors, doc.get("calendarColors") || {});
            console.log("[icloud] Config loaded - busy calendars:", Array.from(busyCalendarUrls).length);
        }
        else {
...
async function initFromEnvIfPossible() {
    ...
    if (!appleId || !appPassword) {
        console.log("[icloud] No Apple ID or app password in env, calendar features disabled");
        return;
    }
    try {
...
router.post("/config", requireAdmin, async (req, res) => {
    ...
    if (Array.isArray(busy)) {
        busyCalendarUrls.clear();
        busy.forEach((u) => busyCalendarUrls.add(u));
    }
...
router.post("/whitelist", requireAdmin, async (req, res) => {
    ...
    if (action === "add")
        whitelistUIDs.add(uid);
    else if (action === "remove")
        whitelistUIDs.delete(uid);
    else
...
router.post("/event-busy", requireAdmin, async (req, res) => {
    ...
    if (!uid || !action)
        return res.status(400).json({ error: "uid_and_action_required" });
    if (action === "add")
        busyEventUIDs.add(uid);
    else if (action === "remove")
        busyEventUIDs.delete(uid);
    else
        return res.status(400).json({ error: "invalid_action" });
    await persistConfig();
    res.json({
...
router.get("/today", requireAdmin, async (_req, res) => {
    ...
    try {
        ...
        const objects = await client.fetchCalendarObjects({
            calendar: { url: calendarHref },
        });
        const dayStart = new Date();
        dayStart.setHours(0, 0, 0, 0);
        const dayEnd = new Date(dayStart.getTime() + 86400000);
        const events = collectEventsFromObjects(objects, dayStart, dayEnd).map((e) => addBlocking(e));
        res.json({ events });
    }
    catch (e) {
        ...
        for (const cal of calendars) {
            try {
                ...
                const events = eventsRaw.filter((ev) => {
                    if (busyCalendarUrls.has(cal.url))
                        return true; // busy calendar -> show
                    if (ev.uid && busyEventUIDs.has(ev.uid))
                        return true; // forced busy -> show
...
router.get("/week", requireAdmin, async (req, res) => {
    ...
    try {
        ...
        for (const cal of calendars) {
            try {
                console.log(`[icloud] Fetching events from calendar: ${cal.displayName}`);
                const calendarObj = cal.raw || { url: cal.url };
                const objects = await client.fetchCalendarObjects({
                    calendar: calendarObj,
                    timeRange: { start: from.toISOString(), end: to.toISOString() },
                ...
                console.log(`[icloud] Calendar ${cal.displayName}: ${(objects || []).length} objects`);
                if (debug) {
                    debugInfo.push({
                        calendar: cal.displayName,
                        objectCount: (objects || []).length,
                ...
                const eventsRaw = collectEventsFromObjects(objects, from, to)
                    ...
                    .map((ev) => addBlocking(ev));
                console.log(`[icloud] Calendar ${cal.displayName}: ${eventsRaw.length} events after processing`);
    ...
    try {
        ...
        for (const cal of calendars) {
            try {
                ...
                const eventsRaw = collectEventsFromObjects(objects, startWindow, endWindow)
                    .map((ev) => ({
                    ...ev,
                    calendar: cal.displayName,
                    calendarUrl: cal.url,
...
function collectEventsFromObjects(objects, windowStart, windowEnd) {
    const events = [];
...
function normalizeRawEvent(cur) {
    const start = parseDate(cur.DTSTART);
    let end = cur.DTEND ? parseDate(cur.DTEND) : undefined;
    if (!end && cur.DURATION) {
...
function expandRecurring(ev, windowStart, windowEnd) {
    ...
    if (!ev.rrule)
        return results;
    const parts = Object.fromEntries(ev.rrule.split(";").map((p) => {
        const [k, v] = p.split("=");
    ...
    if (freq === "DAILY") {
        ...
        while (weekCursor < maxEnd && (!count || occurrence < count)) {
            ...
            for (const dcode of days) {
                ...
                occ.setHours(baseStart.getHours(), baseStart.getMinutes(), baseStart.getSeconds(), baseStart.getMilliseconds());
                if (occ < ev.start)
                    continue; // skip before original DTSTART
                addOcc(occ);
                if (count && occurrence >= count)
                    break;
            }
...
function addBlocking(ev) {
    ...
    let blocking = false;
    if (uid) {
        if (whitelistUIDs.has(uid))
            blocking = false; // explicit allow
        else if (busyEventUIDs.has(uid))
            blocking = true; // forced busy
        else if (calendarBusy)
            blocking = true; // calendar-level busy
    }
    else if (calendarBusy) {
