...
router.get("/week", requireAdmin, async (req, res) => {
    ...
    if (sources.has("icloud"))
        fetches.push(pull("icloud", "/api/icloud/week"));
    if (sources.has("google"))
        fetches.push(pull("google", "/api/google/week"));
    ...
    if (blockingOnly)
        merged = merged.filter((e) => e.blocking === true);
